// NOTER FRA NIELS:
// I kan tjekke crate'en `colored`, til at få nice farver på infotekst.
...
// Som alternativ til en masse macro-kald, kan vi her gemme den fulde fejlbesked som en `&'static str`
// ... med denne lifetime, bliver teksten skrevet direkte, råt ind i den compilede binary. :)
...
fn model(app: &App) -> Model {
    ...
    let mut stars: Vec<hns::Star> =
        initialise_stars(number_of_stars, radius_of_cluster, [0.5, 0.5, 1.0]);
    let cluster_center = hns::Hector {
        x: 0.0,
        y: 0.0,
        z: 0.0,
    };
    let cluster_vel = hns::Hector {
        x: 0.0,
        y: 0.0,
        z: 0.0,
    };
    ...
    let mut stars2: Vec<hns::Star> =
        initialise_stars(number_of_stars, radius_of_cluster, [1.0, 0.5, 0.5]);
...
fn view(app: &App, m: &Model, frame: &Frame) {
    // NIELS: Her ville jeg måske bare bruge `if`, i stedet for at pakke dem ind i en match
        ...
        id if id == m.cam_xy => {
            ...
            for sector in &m.sectors {
                for star in &sector.star_list {
                    draw_xy
                        .ellipse()
                        .x_y(star.pos.x / m.divider, star.pos.y / m.divider)
                        .radius(star.mass)
                        .color(Rgb::new(star.color[0], star.color[1], star.color[2]));
                }
            }
        ...
        id if id == m.cam_xz => {
            ...
            for sector in &m.sectors {
                for star in &sector.star_list {
                    draw_xz
                        .ellipse()
                        .x_y(star.pos.x / m.divider, star.pos.z / m.divider)
                        .radius(star.mass)
                        .color(Rgb::new(star.color[0], star.color[1], star.color[2]));
                }
            }
        ...
        id if id == m.cam_yz => {
            ...
            for sector in &m.sectors {
                for star in &sector.star_list {
                    draw_yz
                        .ellipse()
                        .x_y(star.pos.y / m.divider, star.pos.z / m.divider)
                        .radius(star.mass)
                        .color(Rgb::new(star.color[0], star.color[1], star.color[2]));
                }
            }
...
fn set_center_and_vel(
    stars: &mut Vec<hns::Star>,
    cluster_center: hns::Hector,
    cluster_vel: hns::Hector,
) {
    for star in stars {
...
fn make_sectors(mut star_list: Vec<hns::Star>, recursions_left: u32) -> Vec<hns::Sector> {
    if recursions_left > 0 {
        if 2u32.pow(recursions_left) as usize > star_list.len() {
            panic!(
                "The recursion depth {:?} is greater than the number of stars {:?}",
                recursions_left,
                star_list.len()
            );
        }
