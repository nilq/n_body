// NOTER FRA NIELS:
// - Jeg har udskiftet den implementerede type med `Self`, overhovedet ikke et krav men mere **sugar**.
// - Brugte `cargo fmt` til at formattere koden.
// - Nogen steder kunne man godt have undladt semikolon, ved f.eks. `+=` m.m., når det står sidst i en funktion.
// ... Men det er nogenlunde smag og behag, da man alligevel skal tilføje semikolon, hvis man skriver videre på funktionen.
// Har også implementeret default `Debug` på `Hector` og `Star` - så slipper man for manuelt at printe stats. Sorry, hvis det var intentionen at de skulle
// ... printes anderledes.

#[derive(Copy, Clone, Debug)]
pub struct Hector {
    // Mathematical vector is called Hector in order not to confuse it with a Rust
...
impl PartialEq for Hector {
    //Makes it so you can see if A and B's xyz are the same with A == B
...
impl Star {
    ...
    pub fn find_pos(&mut self, timestep: f32) {
        // Simple Euler integration: s = s + v*dt
...
impl Sector {
    ...
    pub fn add_star(&mut self, new_star: Star) {
        //Adds a single star to a Sector.
        self.total_mass += new_star.mass;
        self.center.add_change(
            &new_star
                .pos
                .multiply(new_star.mass)
                .divide_by(self.total_mass),
        );
        self.star_list.push(new_star);
    }

    pub fn add_multiple_stars(&mut self, new_stars: Vec<Star>) {
        //Adds multiple stars to a Sector
    ...
    pub fn as_star(&self) -> Star {
        //Makes a Star with the Sector's properties.
        Star {
    ...
    pub fn acc_reset(&mut self) {
        //Sets the acceleration of all Stars in the Sector to 0.
        for n in 0..self.star_list.len() {
            self.star_list[n].acc = Hector::new();
        }
    }

    pub fn internal_acc(&mut self) {
        //Accelerates all Stars in the Sector towards each other.
        for n in 0..self.star_list.len() {
    ...
    pub fn external_acc(&mut self, target: &Star) {
        //Accelerates all Stars in the Sector
